SET @old_syphilis_test_concept_id               = (SELECT concept_id FROM concept WHERE uuid = '7ff67b85-7c94-49f6-8187-786fb8f9dbdc');
SET @old_syphilis_test_result_value_id          = (SELECT concept_id FROM concept WHERE uuid = 'ef2bfd0f-db05-450c-8e1c-280a2c4d7a5e');
...
FROM obs o
WHERE o.voided = 0
  AND o.concept_id  = @old_syphilis_test_concept_id
  AND o.value_coded = @old_syphilis_test_result_value_id
UNION
SELECT o.*
...
WHERE o.encounter_id = e.encounter_id and e.encounter_type in (@anc_intake_encounter_id, @anc_followup_encounter_id)
  ...
  AND o.concept_id  = @new_syphilis_test_result_concept_id
