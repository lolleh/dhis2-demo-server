SET @old_hiv_test_concept_id            = (SELECT concept_id FROM concept WHERE uuid = '7ff67b85-7c94-49f6-8187-786fb8f9dbdc');
SET @old_hiv_test_result_value_id       = (SELECT concept_id FROM concept WHERE uuid = '9f2bc0b1-3c77-4850-9fbc-9d138813fa4c');
...
FROM obs o
WHERE o.voided = 0
  AND o.concept_id  = @old_hiv_test_concept_id
  AND o.value_coded = @old_hiv_test_result_value_id
UNION
SELECT o.*
...
WHERE o.encounter_id = e.encounter_id and e.encounter_type in (@anc_intake_encounter_id, @anc_followup_encounter_id)
  ...
  AND o.concept_id  = @new_hiv_test_result_concept_id
