...
set @surgeryEncounterTypeId = (select encounter_type_id from encounter_type where uuid = 'c4941dee-7a9b-4c1c-aa6f-8193e9e5e4e5');
...
create temporary table temp_litre_obs_to_update as
  select o.* from obs o inner join encounter e on o.encounter_id = e.encounter_id
   where o.concept_id in (@volumeMeasurementUrineOld, @estimatedBloodLossOld, @ivfVolumeOld)
     and o.voided = 0
     and e.encounter_type = @surgeryEncounterTypeId;
...
select
    ...
    value_drug,
    value_datetime,
    if(value_numeric < 4, value_numeric * 1000, value_numeric),
    value_modifier,
    value_text,
    value_complex,
    comments,
...
set o.voided = 1, o.date_voided = @currentDate, o.voided_by = @daemonUser, o.void_reason = 'SL-1201: Migrate surgery obs from litre to mL concepts'
